# -*- coding: utf-8 -*-
"""
Created on Sun Dec 28 20:54:48 2025
...
gse_id = 'GSE253822'
out_dir = './test/'
int_set = {'raw','count','matrix','.mtx'}
frag_set = {'rpkm','fpkm','tpm','cpm'}
bad_set = {'ercc', 'spike', 'phix', 'rrna', 'gfp', 'rfp', 'mcherry', 'mt-', 'summary',
                     'report', 'statistics', 'sample_sheet', 'bed', 'bedgraph','peak','hepa', 'cluster'}
comment_set = tuple(['!','#','notes','summary','%','^'])
bad_ext_set = {'.bed','.bedgraph','.wig','bigwig','.bam','.sam','.gtf','.vcf','.soft',
...
def download_process():
    ...
    for fn in files:
        low = fn.lower()
        if any(s in low for s in bad_set):
            continue
        if any(s in low for s in frag_set) or any(s in low for s in int_set):
            url = base_url+fn
    ...
    if not paths:
        ...
        sys.exit(0)
    df_cond = pd.read_csv(f'{gse_id}_meta_gsm_condition.tsv',sep='\t',header=0)
    gsms = set(df_cond['geo_accession'])
    for ii,path1 in enumerate(paths): 
        print(f'No.{ii}: {path1} in processing...')
        mul,flname = decompress_file(path1)
        if mul:
            cmb_df = gsm_cmb(gsms,flname) # assume 1st col is gene
        else:
            table = gsm_reader(str(flname),gse_id) # assume 1st col is gene
    ...
    with os.scandir(flname) as cur:
        for entry in cur: 
            if entry.is_file() and gsm_pat.search(entry.name):
                ...
                if gsm in gsms:
                    if any(entry.name.lower().endswith(c) for c in bad_ext_set):
                        continue
                    if any(c in entry.name.lower() for c in int_set) or any(c in entry.name.lower() for c in frag_set):
                        table = gsm_reader(entry.path,gsm) # dataframe
                        gsm_count_colInd = 0
                        if table is not None and table.shape[1]>1:
                            gsm_count_colInd = guess_count_col(table)
...
def gsm_reader(path1,gsm):
    ...
    if b'\x00' in head1024: # binary test   
        ...
        with os.scandir(Path(path1).parent) as cur:
            for entry in cur:
                if entry.is_file() and gsm.lower() in entry.name.lower() and 'feature' in entry.name.lower():
                    genes_cnt = gsm_reader(entry.path,gsm)
                    break
        ...
        if sparse_mtx.shape[0]!=genes_cnt.shape[0]:
            ...
            return None 
        res = sparse_mtx.sum(axis=1) if is_integer else sparse_mtx.mean(axis=1)
        cnt_cl = pd.DataFrame()
        cnt_cl[0],cnt_cl[1] = genes_cnt.index,res.A1
        return process_colname_index(cnt_cl)
    else: # utf-8
    ...
    cmt_mask = ~cnt_cl.iloc[:,0].astype(str).str.lower().str.startswith(comment_set)
    cnt_cl = cnt_cl[cmt_mask].reset_index(drop=True)
    return process_colname_index(cnt_cl)

def process_colname_index(cnt_cl):
...
def guess_count_col(table):
    count_col = {'count','matrix','rpkm','tpm','cpm','fpkm'}
    colname_which = None 
    for i,colname in enumerate(table.columns):
        if any(c in colname.lower() for c in count_col):
            colname_which = i
            return i
    if not colname_which:
        ...
        colname_which = val-1
        return val-1
    return colname_which
def reorder(cmb_df,df_cond):
    if df_cond['geo_accession'].to_list()!=cmb_df.columns.to_list():
        ...
        if len_cmb<len_cond:
            df_cond.to_csv(f"{gse_id}_meta_gsm_condition.tsv",sep='\t',index=False)
            df_ctrl = pd.read_csv(f'{gse_id}_control_experiments.tsv',sep='\t',engine='python',header=None)
